...
module.exports = async (pubspec, arg) => {
  let suggestedPackages = []
  let selectedPackage = null

  if (arg in pubspec.dependencies) {
    const promptResponse = await prompts({
      message: `Package ${utils.green(arg)} with version ${utils.green(pubspec.dependencies[arg] || 'latest')} already exists in ${utils.green('pubspec.yaml')}. Do you want to upgrade it?`,
      type: 'select',
      name: 'value',
  ...
  try {
    ...
    const promptResponse = await prompts({
      ...
      type: 'select',
      name: 'value',
      choices: [{ title: 'Latest', value: lastVersions[0] }, ...lastVersions.map((v) => { return { title: v, value: v } })],
      initial: 0,
    })

    if(promptResponse.value) {
      const version = promptResponse.value
